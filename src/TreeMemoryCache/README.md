...
- `SetTree<T>(path, value, tag, options)`：按树路径写入并指定标签
...
- `GetPathsByTag(tag)`：查询所有带有指定标签的路径
- `RemoveByTag(tag)`：删除所有带有指定标签的缓存项（包括级联删除子树）
...
batch.Set("Line:6:New", "new-value")
     .Set("temp:report", data, "temp") // 带标签批量写入
     .Remove("Line:6:Old")
     .RemoveTree("Line:8");
batch.Execute();
```
...
- 标签仅设置在你调用 `SetTree` 的目标节点上，任意层级均可设置标签
